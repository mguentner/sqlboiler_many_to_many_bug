...
func (o *Item) GetLists() ListSlice {
	if o == nil {
		return nil
	}
...
func (r *itemR) GetLists() ListSlice {
	if r == nil {
		return nil
	}
...
func (o *Item) Tags(mods ...qm.QueryMod) tagQuery {
	...
	queryMods = append(queryMods,
		qm.InnerJoin("\"item_tags\" on \"tags\".\"id\" = \"item_tags\".\"tag_id\""),
		qm.Where("\"item_tags\".\"item_id\"=?", o.ID),
	)
...
func (o *Item) Lists(mods ...qm.QueryMod) listQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.InnerJoin("\"list_items\" on \"lists\".\"id\" = \"list_items\".\"list_id\""),
		qm.Where("\"list_items\".\"item_id\"=?", o.ID),
	)
...
func (itemL) LoadTags(ctx context.Context, e boil.ContextExecutor, singular bool, maybeItem interface{}, mods queries.Applicator) error {
	var slice []*Item
	var object *Item
	...
	query := NewQuery(
		qm.Select("\"tags\".\"id\", \"tags\".\"name\", \"a\".\"item_id\""),
		qm.From("\"tags\""),
		qm.InnerJoin("\"item_tags\" as \"a\" on \"tags\".\"id\" = \"a\".\"tag_id\""),
		qm.WhereIn("\"a\".\"item_id\" in ?", argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (itemL) LoadLists(ctx context.Context, e boil.ContextExecutor, singular bool, maybeItem interface{}, mods queries.Applicator) error {
	var slice []*Item
	var object *Item
	...
	query := NewQuery(
		qm.Select("\"lists\".\"id\", \"lists\".\"name\", \"a\".\"item_id\""),
		qm.From("\"lists\""),
		qm.InnerJoin("\"list_items\" as \"a\" on \"lists\".\"id\" = \"a\".\"list_id\""),
		qm.WhereIn("\"a\".\"item_id\" in ?", argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
...
// Sets related.R.Items appropriately.
func (o *Item) AddTags(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Tag) error {
	var err error
	...
	for _, rel := range related {
		query := "insert into \"item_tags\" (\"item_id\", \"tag_id\") values ($1, $2)"
...
// Sets o.R.Items's Tags accordingly.
// Replaces o.R.Tags with related.
// Sets related.R.Items's Tags accordingly.
func (o *Item) SetTags(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Tag) error {
	query := "delete from \"item_tags\" where \"item_id\" = $1"
	...
	removeTagsFromItemsSlice(o, related)
	if o.R != nil {
		o.R.Tags = nil
	}
...
// Sets related.R.Items.
func (o *Item) RemoveTags(ctx context.Context, exec boil.ContextExecutor, related ...*Tag) error {
	if len(related) == 0 {
		return nil
	}
	...
	var err error
	query := fmt.Sprintf(
		"delete from \"item_tags\" where \"item_id\" = $1 and \"tag_id\" in (%s)",
...
func removeTagsFromItemsSlice(o *Item, related []*Tag) {
	for _, rel := range related {
		if rel.R == nil {
			continue
		}
		for i, ri := range rel.R.Items {
			if o.ID != ri.ID {
				continue
			}
			...
			ln := len(rel.R.Items)
			if ln > 1 && i < ln-1 {
				rel.R.Items[i] = rel.R.Items[ln-1]
			}
			rel.R.Items = rel.R.Items[:ln-1]
			break
		}
	}
}
...
// Sets related.R.Items appropriately.
func (o *Item) AddLists(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*List) error {
	var err error
	for _, rel := range related {
		if insert {
			if err = rel.Insert(ctx, exec, boil.Infer()); err != nil {
	...
	for _, rel := range related {
		query := "insert into \"list_items\" (\"item_id\", \"list_id\") values ($1, $2)"
		values := []interface{}{o.ID, rel.ID}
...
// item replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.Items's Lists accordingly.
// Replaces o.R.Lists with related.
// Sets related.R.Items's Lists accordingly.
func (o *Item) SetLists(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*List) error {
	query := "delete from \"list_items\" where \"item_id\" = $1"
	values := []interface{}{o.ID}
	if boil.IsDebug(ctx) {
		writer := boil.DebugWriterFrom(ctx)
		fmt.Fprintln(writer, query)
	...
	removeListsFromItemsSlice(o, related)
	if o.R != nil {
		o.R.Lists = nil
	}
...
// Sets related.R.Items.
func (o *Item) RemoveLists(ctx context.Context, exec boil.ContextExecutor, related ...*List) error {
	if len(related) == 0 {
		return nil
	}
	...
	var err error
	query := fmt.Sprintf(
		"delete from \"list_items\" where \"item_id\" = $1 and \"list_id\" in (%s)",
		strmangle.Placeholders(dialect.UseIndexPlaceholders, len(related), 2, 1),
	)
	values := []interface{}{o.ID}
	for _, rel := range related {
...
func removeListsFromItemsSlice(o *Item, related []*List) {
	for _, rel := range related {
		if rel.R == nil {
			continue
		}
	...
	if !cached {
		...
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(itemPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert items, could not build conflict column list")
			}
